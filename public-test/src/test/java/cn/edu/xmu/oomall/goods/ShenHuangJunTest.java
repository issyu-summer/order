...
public class ShenHuangJunTest {
    ...
    public void getAllCommnetOfUser1() throws Exception{
        String token = this.login("13088admin", "123456");
        byte[] responseBuffer=mallClient.get().uri("/skus/273/comments?page=1&pageSize=10")
    ...
    public void getAllCommnetOfUser2() throws Exception{
        String token = this.login("13088admin", "123456");
        byte[] responseBuffer=mallClient.get().uri("/skus/273/comments?page=2&pageSize=10")
                .header("authorization", token).exchange()
                .expectStatus().isOk()
                .expectHeader().contentType("application/json;charset=UTF-8")
                .expectBody()
    ...
    public void getAllCommnetOfUser3() throws Exception{
        byte[] responseString = mallClient.get().uri("/skus/273/comments?page=1&pageSize=10")
                .exchange()
                .expectStatus().isUnauthorized()
                .expectBody()
                .returnResult()
