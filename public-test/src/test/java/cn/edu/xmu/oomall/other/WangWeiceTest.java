...
public class WangWeiceTest {
    ...
    public void userTest21() throws Exception {
        ...
        byte[] responseString = mallClient.post().uri("/users").bodyValue(requireJson).exchange()
                ...
                .returnResult()
                .getResponseBodyContent();
        String expectedResponse = "{\"errno\":0,\"data\":{\"userName\":\"testU21\",\"name\":\"testR21\",\"mobile\":\"12332112321\",\"email\":\"2121@2121.com\",\"gender\":0,\"birthday\":\"2020-12-09\",\"state\":4}}";
        JSONAssert.assertEquals(expectedResponse, new String(responseString, StandardCharsets.UTF_8), false);
        log.debug("尝试登录");
        body = new JSONObject();
        body.put("userName", "testU21");
        ...
        responseString = mallClient.get().uri("/users").header("authorization", token).exchange()
                ...
                .returnResult()
                .getResponseBodyContent();
        expectedResponse = "{\"errno\":0,\"data\":{\"userName\":\"testU21\",\"name\":\"testR21\",\"mobile\":\"12332112321\",\"email\":\"2121@2121.com\",\"gender\":0,\"birthday\":\"2020-12-09\",\"state\":4}}";
        JSONAssert.assertEquals(expectedResponse, new String(responseString, StandardCharsets.UTF_8), false);
        mallClient.get().uri("/users/logout").header("authorization", token).exchange()
                .expectStatus().isOk()
                .expectBody()
