...
public class XiangSuXianTest {
    ...
    public void getPresales1() throws Exception {
        byte[]responseString=mallClient.get().uri("/presales?shopId=2")
                .exchange()
                .expectStatus().isOk()
                .expectHeader().contentType("application/json;charset=UTF-8")
                .expectBody()
...
//        byte[] responseString = manageClient.delete().uri("/goods/shops/1/presales/9")
...
//        String token = this.login("13088admin", "123456");
//        byte[] responseString = manageClient.delete().uri("/goods/shops/1/groupons/1")
//                .header("authorization", token)
//                .exchange()
//                .expectStatus().isOk()
//                .expectBody()
...
//删除团购（团购状态禁止）
    ...
    public void deleteGroupon2() throws Exception {
        String token = this.login("13088admin", "123456");
        byte[] responseString = manageClient.delete().uri("/shops/2/groupons/4")
                .header("authorization", token)
                .exchange()
                .expectStatus().isOk()
                .expectBody()
